...
def add_fertilizante_features(df: pd.DataFrame) -> pd.DataFrame:
    """Adiciona fert_total_safra_uf (ton fertilizante por UF-ano)."""
    ...
    df = df.merge(
        df_fert.rename(columns={"fert_total_ton": "fert_total_safra_uf"}),
        left_on=["_uf_cod", "ano"],
        right_on=["uf_cod", "ano"],
        how="left",
    )
    ...
    n_with = df["fert_total_safra_uf"].notna().sum()
...
def add_new_source_interactions(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "fert_total_safra_uf" in df.columns and "precip_anomaly" in df.columns:
        fert_norm = df["fert_total_safra_uf"].fillna(0) / (df["fert_total_safra_uf"].std() + 1e-8)
